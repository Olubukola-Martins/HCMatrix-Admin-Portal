...
export const listOfEmailValidationRule: Rule = {
  validator: async (_, value) => {
    // non required
    if (typeof value !== "string") {
      throw new Error(
        "Please enter a valid list of emails seperated by comma!"
      );
    }
    ...
    emailValues.forEach((item, i) => {
      if (isEmailValid(item) === false) {
        throw new Error(`Please enter a valid email at position ${i + 1}`);
      }
    });
...
export const createFileValidationRule = (
  ...
  return {
    ...
    validator: async (_, value) => {
      ...
      (value as Record<string, File>[]).forEach((item, i) => {
...
export const urlValidationRule: Rule = {
  validator: async (_, value) => {
    ...
    const validUrlPattern =
      /^https:\/\/([\da-z.-]+)\.([a-z.]{2,6})([/\w .-]*)*\/?$/; //note this pattern will only allow 4 https and not http or string | number other protocols
...
const isValidVariableName = (name: string) => {
  const reservedKeywords = [
    // Add string | number additional reserved keywords here
    "break",
    "case",
    "catch",
    "class",
