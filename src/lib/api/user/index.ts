...
const getData = async (
...
): Promise<TApiResponseWithPagination<TUser>> => {
  const url = `/user`;
  ...
  const res = response.data as TApiResponseWithPagination<TUser>;
  return res;
};

export const useGetUsers = (props: IGetDataProps = {}) => {
  const { pagination, search } = props;
  const queryData = useQuery(
    [QUERY_KEY_FOR_USERS, pagination, search],
    () =>
      getData({
        data: {
          ...props,
